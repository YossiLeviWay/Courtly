...
// One-time admin endpoint to generate the shared game world server-side with
// deterministic (seeded) data so every installation produces the same teams,
...
//
// Set ADMIN_SECRET in Vercel environment variables (Project Settings → Environment Variables).

const { neon } = require('@neondatabase/serverless');

// ── Seeded PRNG (mulberry32) ────────────────────────────────────
// Always produces the same sequence for seed=42 → same world every time.
let _rngState = 42;
...
// 50 unique teams — 10 per league, each with a fixed city & name
...
function generatePlayer(pos, teamIdx, playerIdx) {
  const baseLevel = 42 + (teamIdx % 10) * 2; // slight variance per team
  const attributes = generateAttributes(baseLevel);
  const overall = Math.round(
    Object.values(attributes).reduce((s, v) => s + v, 0) / ATTRIBUTE_KEYS.length
  );
...
function generateTeam(def, idx) {
  const leagueIdx = Math.floor(idx / 10);
  const positions = ['PG', 'PG', 'PG', 'SG', 'SG', 'SG', 'SF', 'SF', 'SF', 'PF', 'PF', 'PF', 'C', 'C', 'C'];
  const players = positions.map((pos, pIdx) => generatePlayer(pos, idx, pIdx));
  return {
    id: `team-${idx}`,
    name: def.name,
...
function generateSchedule(teams, leagueIndex) {
  ...
  const selected = shuffle(pairs).slice(0, 90); // 18 games per team
...
function buildWorld() {
  ...
  const leagues = [];
  for (let i = 0; i < 5; i++) {
    const teamDefs = TEAM_DEFINITIONS.slice(i * 10, i * 10 + 10);
    const teams = teamDefs.map((def, j) => generateTeam(def, i * 10 + j));
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!reset) {
      ...
      if (count > 0) {
        return res.json({ message: 'World already seeded. Pass reset:true to re-seed.', seeded: false });
      }
    } else {
      // Wipe shared tables (keeps users + user_team_state)
    ...
    console.log(`Generated ${allTeams.length} teams, ${allMatches.length} matches`);
    ...
    return res.json({
      message: `World seeded: ${allTeams.length} teams across ${leagues.length} leagues, ${allMatches.length} matches scheduled.`,
      seeded: true,
