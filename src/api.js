...
function normalizeUserState(s) {
  ...
  return {
    ...
    seasonTicketSeats:   s.seasonTicketSeats   ?? 0,
    seasonTicketPrice:   s.seasonTicketPrice   ?? 15,
    awayFanPct:          s.awayFanPct          ?? 10,
...
export async function apiSaveUserState(payload) {
  ...
  try {
    ...
    await setDoc(doc(db, 'user_team_state', uid), {
      ...
      seasonTicketSeats:   payload.seasonTicketSeats   ?? 0,
      seasonTicketPrice:   payload.seasonTicketPrice   ?? 15,
      awayFanPct:          payload.awayFanPct          ?? 10,
